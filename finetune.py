...
if __name__ == '__main__':
    ...
    checkpoint = torch.load("pretrained_models/MFF_jester_RGBFlow_BNInception_segment4_3f1c_best.pth.tar")
    print("model epoch {} best prec@1: {}".format(checkpoint['epoch'], checkpoint['best_prec1']))

    base_dict = {'.'.join(k.split('.')[2:]): v for k, v in list(checkpoint['state_dict'].items())[:-6]}
    model.base_model.load_state_dict(base_dict)
...
#    print("Children:", len(list(model.modules())))
#    for name, child in model.named_children():
#        print("    Child:", name.ljust(30), ":", child)
#    print("-----------------------------------------------------------------------------------------------------------------")


    # reset the weight for last layers
    torch.nn.init.xavier_uniform(model.new_fc.weight.data)
    torch.nn.init.normal(model.new_fc.bias.data)

    for module in model.consensus.modules():
        if isinstance(module, torch.nn.Linear):
            torch.nn.init.xavier_uniform(module.weight.data)
            torch.nn.init.normal(module.bias.data)

    print("MODEL CREATED, STARTING FINETUNING")


    print(model.crop_size, model.scale_size, model.input_mean, model.input_std)
